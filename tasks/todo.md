...
- [ ] 1. Delegate lifetime (R1-H2, CONFIRMED high): TokenMyBarApp.swift:13-17 —
  `withExtendedLifetime(delegate) { app.run() }`. Verify: build -c release, launch,
  status item appears.
- [ ] 2. Claude NaN trap (R2-2, CONFIRMED, crash reproduced): ClaudeOAuthUsageProvider
  ...
  huge-finite.
- [ ] 3. Unguarded private KVC (R2-10b): TokenMyBarApp.swift:57 — wrap
...
- [ ] 4. Antigravity fallback composition (R2-1): FallbackProviderTests — new red test:
...
- [ ] 5. Local providers `.noData` beside real data (R2-8a/b): key status on the same
...
- [ ] 6. Vendor vanishes with no glyph (R1-M6, corrected): headline drop for elapsed
...
- [ ] 7. OpenCode schema drift eaten (R2-5): OpenCodeCookieUsageProvider loop — keep
...
- [ ] 8. CLI contract (R1-M3 corrected + disabled-vendors, both real): (a) reserve
...
- [ ] 9. Settings privacy copy (R2-3): SettingsWindow.swift:209 → "TokenMyBar reads
...
- [ ] 10. Antigravity docs sweep (R1-M7/R2-14, 8 confirmed spots): architecture.md:75-82,
...
- [ ] 11. CI gates (R2-13/13b/low): `prettier --write` the 5 failing www files; www
...
- [ ] 12. BlockingIO serial queue (real kernel of R1-H1): drop `.concurrent` from
...
- [ ] 13. Route blocking work through BlockingIO (R2-7): Antigravity lsof `portFinder`,
...
- [ ] 14. Settings honesty (R1-M4/R2-9): remove `DisplayMode.custom` (raw "custom"
...
- [ ] 15. Swift misc: PopoverView.swift:468 a11y value uses `.rounded()` via one shared
...
- [ ] 16. www fixes: figcaption alpha 0.45→0.55 (5.45:1, passes AA) at
...
- [ ] 17. install.sh: normalize `version="${version#v}"` so `v1.0.7` works (+bash test).
